...
const processCommand = async (message) => {
  ...
  const cmdArguments = splitCommand.slice(1);

  switch (primaryCommand.toLowerCase()) {
    case 'bug':
      if (cmdArguments.length > 0) {
        const bug = await cmdArguments.join(' ');
        etrizDM(bug);
      ...
      break;
    case 'help':
      helpCommand(cmdArguments, message);
      break;
    case 'next':
    ...
    case 'addcommand':
      if (cmdArguments.length === 0) {
        ...
        if (cmdArguments.length > 1) {
          const newCommand = cmdArguments[0];
          const temp = cmdArguments.slice(1);
    ...
    case 'set':
      ...
      } else if (cmdArguments[0].toLowerCase() === 'next') {
        if (cmdArguments.length > 1) {
          let temp = cmdArguments.slice(1);
      ...
      } else if (cmdArguments[0].toLowerCase() === 'activity') {
        if (message.author.id === etriz) {
          client.user.setActivity(cmdArguments.slice(2).join(' '), {
            type: cmdArguments[1].toUpperCase(),
          });
        } else {
...
const helpCommand = (cmdArguments, message) => {
  cmdArguments.length > 0
...
const bitCoinCommand = async (message) => {
  ...
  const res = await axios.get(
    'https://api.coingecko.com/api/v3/simple/price?ids=ethereum%2Cbitcoin%2Cdogecoin&vs_currencies=usd'
  );
  const bitcoin = await res.data.bitcoin.usd;
  const eth = await res.data.ethereum.usd;
  const doge = await res.data.dogecoin.usd;
  if (bitcoin && eth && doge) {
    search.delete();
    message.reply(
      `Bitcoin price: ${formatCurrency(bitcoin)}\nEthereum price: ${formatCurrency(
        eth
      )}\nDogecoin price: ${formatCurrency(doge)}`
    );
  }
};
